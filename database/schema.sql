-- ═══════════════════════════════════════════════════════
--  OmegaExchange — Supabase Database Schema
...
create table if not exists public.users (
    id          text primary key,         -- omegacases userid
...
create table if not exists public.deposits (
    ...
    reference   text not null,           -- PayPal TX ID or gift card code
...
create table if not exists public.offers (
    ...
    price                numeric not null check (price > 0),  -- USD per unit
    min_amount           numeric not null check (min_amount > 0),  -- USD
    max_amount           numeric not null check (max_amount >= min_amount),  -- USD
    available_amount     numeric not null check (available_amount >= 0),  -- crypto units
...
create table if not exists public.trades (
    ...
    currency      text not null default 'BTC',
    amount        numeric not null check (amount > 0),   -- crypto amount
    usd_amount    numeric,                               -- USD total
    price         numeric not null check (price > 0),   -- price at trade time
...
create table if not exists public.notifications (
    ...
    type         text,                  -- 'trade', 'deposit', 'dispute', 'system'
    reference_id text,                  -- trade ID or deposit ID
...
-- Increment reputation after completed trade
...
-- Restore offer amount on cancel
...
begin
    select * into v_deposit from public.deposits where id = p_deposit_id and status = 'pending';
